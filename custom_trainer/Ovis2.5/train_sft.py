...
from transformers import (
    AutoModelForCausalLM,  # Ovis uses AutoModelForCausalLM
...
class OvisSFTTrainer:
    ...
    def load_model(self):
        """Load Ovis2.5-9B model (no separate tokenizer needed)"""
        ...
        # Load model using AutoModelForCausalLM (key difference from Qwen)
        self.model = AutoModelForCausalLM.from_pretrained(
            self.model_config.model_name, **model_kwargs
        )

        # Handle custom tokens if specified (for consistency with Qwen data)
        ...
        if self.training_config.use_lora:
            self.apply_lora()
            logger.warning("LoRA should be disabled due to PEFT compatibility issues with Ovis2.5")
            # self.apply_lora()  # Commented out until PEFT compatibility is resolved

    def add_custom_tokens(self):
        """Add custom tool call tokens (same as Qwen implementation)"""
        logger.info("Adding custom tool call tokens")

        # Add custom tokens to Ovis tokenizer
        new_tokens = [
            self.model_config.tool_call_start_token,
            self.model_config.tool_call_end_token,
        ]

        num_added_tokens = self.model.text_tokenizer.add_tokens(new_tokens)
        logger.info(f"Added {num_added_tokens} new tokens")

        # Update token IDs in config
        vocab = self.model.text_tokenizer.get_vocab()
        self.model_config.tool_call_start_token_id = vocab.get(
            self.model_config.tool_call_start_token, None
        )
        self.model_config.tool_call_end_token_id = vocab.get(
            self.model_config.tool_call_end_token, None
        )

        # Handle token embedding resizing for Ovis2.5 model
        if num_added_tokens > 0:
            try:
                # Try standard resize method first
                self.model.resize_token_embeddings(len(self.model.text_tokenizer))
                logger.info("Token embeddings resized using standard method")
            except NotImplementedError:
                # Ovis2.5 specific handling - manually resize embeddings
                logger.info("Using Ovis2.5 specific token embedding resize")
                self._resize_ovis_embeddings(len(self.model.text_tokenizer))
            except Exception as e:
                logger.warning(f"Could not resize token embeddings: {e}")
                logger.info("Continuing without embedding resize - model may need retraining")

        logger.info("Custom tokens added successfully")

    def _resize_ovis_embeddings(self, new_vocab_size: int):
        """Ovis2.5 specific token embedding resize"""
        try:
            # Try to find the embedding layer in the model
            if hasattr(self.model, "model") and hasattr(self.model.model, "embed_tokens"):
                # Common pattern for language models
                old_embeddings = self.model.model.embed_tokens
                ...
                logger.warning("Could not find embedding layer in Ovis2.5 model")
                return
            ...
            if new_vocab_size > old_vocab_size:
                ...
                with torch.no_grad():
                    ...
                    new_embeddings.weight[old_vocab_size:] = (
                        torch.randn(new_vocab_size - old_vocab_size, embedding_dim) * 0.02
                    )
                ...
                logger.info(f"Resized embeddings from {old_vocab_size} to {new_vocab_size}")
            else:
                logger.info("No resize needed - vocabulary size unchanged")

        except Exception as e:
            logger.warning(f"Failed to resize Ovis2.5 embeddings: {e}")
            logger.info("Continuing without embedding resize")
    ...
    def apply_lora(self):
        ...
        # LoRA configuration - adapted for Ovis architecture
    ...
    def prepare_datasets(self):
        ...
        self.train_dataset = OvisDataset(
            ...
            model=self.model,  # Pass model for preprocessing
    ...
    def train(self):
        ...
        data_collator = OvisDataCollator(
            model=self.model,  # Pass model to get correct tokenizer
            padding=True,
        )
...
def main():
    ...
    parser.add_argument("--sft_config", type=str, help="Path to SFT training configuration JSON file")
    ...
    # Output directory
    parser.add_argument("--output_dir", type=str, default="outputs/sft", help="Output directory for SFT stage")
