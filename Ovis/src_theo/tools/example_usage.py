...
def load_model():
    ...
    model = AutoModelForCausalLM.from_pretrained(
        "AIDC-AI/Ovis2.5-9B", trust_remote_code=True, torch_dtype=torch.bfloat16, device_map="auto"
    )
    # model = AutoModelForCausalLM.from_pretrained(
    #     "Ovis/checkpoints/ovis25_ft_refcoco_croptool_v1", trust_remote_code=True, torch_dtype=torch.bfloat16, device_map="auto"
    # )
    return model
...
def example_batch_generation():
    ...
    # Load test image
    image = Image.open("Ovis/src_theo/sample_data/demo_person.jpg")

    # Chat with tool execution
    response, thinking, history = chat_with_tool_execution(
        model=model,
        prompt="<image>Please examine this image in detail. Focus on any people you see.",
        images=[image],
        do_sample=True,
        temperature=0.6,
        max_new_tokens=1024,
...
def example_multi_turn_conversation():
    ...
    # Chat with tool execution
    response1, thinking1, history = chat_with_tool_execution(
        model=model,
    ...
    response2, thinking2, history = chat_with_tool_execution(
        model=model,
        prompt="<image>Can you examine the clothing of any people in more detail?",
        history=history,
        do_sample=True,
        temperature=0.6,
        max_new_tokens=1024,
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("2. Sample image in the correct directory")
